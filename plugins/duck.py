...
from cloudbot.util import database as botvars
...
@hook.command("modify", autohelp=False)
def modd(text, db, conn):
    data = text.split(';')
    nick = data[0]
    chan = data[1]
    score = data[2]
    print("{},{},{}".format(nick, chan, score))
...
def bang(nick, chan, message, db, conn, notice):
    ...
    score = ""
    out = ""
    miss = ["WHOOSH! You missed the duck completely!", "Your gun jammed!", "Better luck next time.", "That was a loss of a duck." ]
    if not game_status[network][chan]['game_on']:
        return "There is no active hunt right now. Use @starthunt to begin."
        ...
        if nick.lower() in scripters:
            if scripters[nick.lower()] > shoot:
                notice("You are in a cooldown period; try again in {} seconds.".format(str(scripters[nick.lower()] - shoot)))
                return
        chance = hit_or_miss(deploy, shoot)
        if not random.random() <= chance and chance > .05:
            out = random.choice(miss) + " Try again in 7 seconds."
            scripters[nick.lower()] = shoot + 7 
            return out
        if chance == .05:
            out += "You pulled the trigger in {} seconds, that's too quick. Maybe it's a script? Take a 2 hour cooldown.".format(str(shoot - deploy))
...
def befriend(nick, chan, message, db, conn, notice):
    ...
    if not game_status[network][chan]['game_on']:
        return "There is no hunt right now. Use @starthunt to begin."
    elif game_status[network][chan]['duck_status'] != 1:
        if game_status[network][chan]['no_duck_kick'] == 1:
            out = "KICK {} {} You tried befriending a non-existent duck, that's very creepy.".format(chan, nick)
            conn.send(out)
            return
        return "You tried befriending a non-existent duck, that's creepy."
    else:
        game_status[network][chan]['shoot_time'] = time()
        deploy = game_status[network][chan]['duck_time']
        shoot = game_status[network][chan]['shoot_time']
...
def ducks_user(text, nick, chan, conn, db, message):
    ...
    if scores:
        ...
        return "{} has not participated in the duck hunt.".format(name)
...
def duck_stats(chan, conn, db, message):
    ...
    if scores:
        ...
        return "There has been no duck activity on this channel/network."
